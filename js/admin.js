function checkLogin() {
    ...
    if (currentUser == null || currentUser.userType == 0) {
        window.location.href = "/Lập trình đồ án ứng ụng 2/Webbanhang-main - Copy/index.html"; 
    } else {
        document.getElementById("name-acc").innerHTML = currentUser.fullname;

        // Kiểm tra trạng thái đơn hàng mới
        if (localStorage.getItem('newOrder') === 'true') {
            console.log('Có đơn hàng mới, hiển thị thông báo!');
            showNotification();
            localStorage.removeItem('newOrder');
        } else {
            console.log('Không có đơn hàng mới.');
        }
    }
}
...
function checkNewOrder() {
    if (localStorage.getItem('newOrder') === 'true') {
        showNotification();
        // Xóa cờ để tránh lặp lại thông báo
        localStorage.removeItem('newOrder');
    }
}
